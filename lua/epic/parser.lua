...
local function parse_rfc2822(text)
  local d, mon, y, h, mi, s, off = text:match(
    "^%a*,%s*(%d%d)%s+(%a+)%s+(%d%d%d%d)%s+(%d%d):(%d%d):(%d%d)%s+([%+%-]%d%d%d%d)$"
  )
  if not d then
    d, mon, y, h, mi, s, off = text:match(
      "^(%d%d)%s+(%a+)%s+(%d%d%d%d)%s+(%d%d):(%d%d):(%d%d)%s+([%+%-]%d%d%d%d)$"
    )
  end
  if not d then
    return nil
  ...
  local t = {
    year = tonumber(y), month = mo, day = tonumber(d),
...
local function parse_http_date(text)
  local d, mon, y, h, mi, s, tz = text:match(

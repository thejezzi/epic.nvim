...
test("format to de date without time", function()
	local v = parser.parse("2026-05-20T12:30:00Z")
	assert_eq(v ~= nil, true, "parsed date cannot be nil")
	local d = formats.format_de_date(v --[[@as types.Date]])
	assert(d:match("^%d%d%.%d%d%.%d%d%d%d$") ~= nil, "de date format: " .. d)
	assert(d:match("^%d%d%.%d%d%.%d%d%d%d %d%d:%d%d:%d%d$") == nil, "de date must not match de datetime")
end)
